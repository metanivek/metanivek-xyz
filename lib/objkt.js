  ...
  return objkts.map((i) => {
    ...
    return {
      ...
      uris: {
        ...
        hen: `https://hen.teztools.io/objkt/${i.id}`,
      },
...
export function primaryCount(items) {
  return items.reduce((m, s) => m + (!s.soldOut && !s.secondary ? 1 : 0), 0);
}
export function secondaryCount(items) {
  return items.reduce((m, s) => m + (s.secondary ? 1 : 0), 0);
}
